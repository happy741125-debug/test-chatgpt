# Work Intelligence Hub Development Tasks V2.2

> 依據：[PRD V2.2](./PRD.md)、[SDD V2.2](./SDD.md) 與 [ADR-0001](./ADR-0001-OPERATIONS-MASTER-BACKBONE.md)
>
> 執行原則：先完成資料層與管理層，再啟用執行層。六個核心管理模組是下一階段主線；AI 不得自行派工、結案、對外承諾或做員工績效評分。
...
以《貨達營運總表》為管理定義來源，依序建立共用來源治理、六個核心管理模組與 Dashboard。管理資料驗收後，才開發由人確認的任務與改善追蹤；不包含 AI 自動派工、SLA 處分、員工績效或對外自動回覆。
...
- [x] **T6.11** 上傳預覽顯示貨主辨識依據與筆數（來源欄位、既有訂單關聯、庫存商品編號、人工選擇或尚未辨識），讓使用者確認歸屬來源。
- [x] **T6.12** 建立未辨識商品清單；管理者可將商品編號指定至正式貨主，後續上傳自動沿用並保留首次、最近出現及處理時間。
...
## V3.6｜簡化上傳與每日訂單完成度（已由 V3.7 修正口徑）

**目標：** 同事只負責提供營運檔案，系統負責分類與補正；中台優先回答「當日訂單是否全部處理完畢」。

- [x] **T36-01** 上傳頁自動辨識訂單、庫存、進倉、退貨、揀貨與托運檔案類型。
- [x] **T36-02** 倉庫只需在同一台裝置選擇一次並記住，支援一次上傳多份檔案。
- [x] **T36-03** 移除同事端貨主選擇與新增貨主流程；無法辨識貨主時仍接受資料並列入後台資料品質清單。
- [x] **T36-04** 新匯入資料保存非敏感商品編號關聯；庫存或商品對照更新後，自動補正相關資料貨主。
- [x] **T36-05** 營運表現優先顯示最新作業日的應處理訂單、已完成、未完成與完成率。
- [x] **T36-06** 複核實際匯出檔：訂單有建立時間、狀態與出貨時間，但預約出貨日覆蓋不足；決定改採 V3.7 每週訂單處理口徑。

**驗收 Gate：** 同事不必判斷檔案類型或貨主；不明貨主不阻塞收件；新增對照後不必重新上傳；每日完成率只依來源檔可驗證欄位計算。

---

## V3.7｜每週訂單處理狀態

**目標：** 以低維護成本回答「本週應處理的訂單完成多少」，並將訂單履約與包裹／PCS 工作量分開。

- [x] **T37-01** 首頁與營運表現改顯示週一至週日的應處理、已完成、未完成與完成率。
- [x] **T37-02** 無預約出貨日時，依訂單建立時間與每日 13:00 結單規則決定處理日；13:00 起歸入下一日。
- [x] **T37-03** 有預約出貨日時以預約日期優先；排除「已取消」；只有訂單狀態等於「已完成」才算完成。
- [x] **T37-04** 包裹、物流單與揀貨 PCS 只作工作量，不作為完成訂單數或完成率。
- [x] **T37-05** GOwarehouse 自動摘要不再顯示資料覆蓋不足的準時率；完成率明示週期、結單時間、資料更新時間與排除取消數。
- [x] **T37-06** 以去識別化測試覆蓋 13:00 邊界、預約日期優先、取消排除，以及「已出貨」不得誤算為「已完成」。

**驗收 Gate：** 同一週以不重複訂單計算；沒有日期不顯示假數字；13:00 邊界正確；取消不進分母；出貨時間或物流狀態不會越過「已完成」標準。

---

## V3.8｜休假日規則

**目標：** 讓每週訂單處理狀態依貨達實際工作日計算，休假日不再被誤列為應處理日。

- [x] **T38-01** 系統設定可選擇每週固定休假日，預設星期日休假。
- [x] **T38-02** 可新增特定休假日（國定假日、公司休假、臨時停班）與補班日。
- [x] **T38-03** 依建立時間與 13:00 結單推算的處理日若遇休假，自動順延到下一工作日。
- [x] **T38-04** 預約出貨日若遇休假，同樣順延到下一工作日；補班日優先覆蓋固定週休。
- [x] **T38-05** 規則保存於後端，重新登入或更換裝置仍會沿用；僅管理密碼可修改。
- [x] **T38-06** 首頁與營運表現明示休假日順延規則，並於儲存後立即重算週完成率。
- [x] **T38-07** 去識別化測試覆蓋預設週休、特定休假、補班、13:00 後跨休假日、日期衝突與權限。

**驗收 Gate：** 至少保留一個每週工作日；同一日期不能同時是休假日與補班日；休假日與補班規則能正確改變訂單所屬處理週；既有「已完成／已取消」口徑不變。

---

## Phase 18｜營運總表承接與共用管理骨架

**目標：** 讓《貨達營運總表》從持續累積的管理來源，安全轉為中台可承載、更新與追溯的結構化管理資料。

- [ ] **T18-01** 建立來源清冊，確認 GoWarehouse、財務、人事、倉容、人工回報與營運總表各自的權威範圍。
- [ ] **T18-02** 建立 `source_registry`、`import_batches`、來源版本、checksum、資料新鮮度與品質狀態。
- [ ] **T18-03** 建立六模組共用欄位與 Audit：狀態、Owner、有效期間、來源、證據、版本、建立／更新時間。
- [ ] **T18-04** 建立跨模組 `management_links`，避免同一問題在 KPI、SOP、Onboarding 重複建檔。
- [ ] **T18-05** 建立匯入預覽、欄位對照、衝突 Review、整批撤銷與重新處理。
- [ ] **T18-06** 定義營運總表過渡策略：尚未上線模組仍由總表管理；已驗收模組由中台成為正式管理介面。
- [ ] **T18-07** 建立管理首頁骨架，顯示六模組摘要、資料更新時間、來源健康與待確認項目。
- [ ] **T18-08** 建立角色權限與敏感資料矩陣，特別處理人員、財務與客戶資料。

**V1 初始切片（2026-09-22）：** 已完成來源清冊／匯入批次／版本與 checksum、六模組共用紀錄與 Audit、同版本衝突阻擋、營運總表首頁骨架，以及「人員盤點」獨立管理者端點與延後載入。人員內容只匯入中台資料庫，不提交公開 GitHub。T18-01～T18-03、T18-07～T18-08 的完整驗收仍須補齊其他資料來源、Owner、資料新鮮度、來源健康及完整角色矩陣，因此暫不勾選整項完成。

**驗收 Gate：** 任一管理資料可追溯來源、版本與匯入批次；衝突不被靜默覆蓋；撤銷批次不破壞歷史 Audit。

---

## Phase 19｜六個核心管理模組

**目標：** 第一版完成管理者最需要的六個資料模組，先能看清狀態與偏差，再考慮完整執行自動化。

- [ ] **T19-01 倉庫容量**：倉別／區域／容量單位、總量、已用、保留、可用、觀測時間、來源與歷史趨勢。
- [ ] **T19-02 營運問題**：領域、嚴重度、影響、原因、狀態、復發、證據及關聯 KPI／SOP。
- [ ] **T19-03 SOP**：SOP 主檔、版本、適用範圍、生效日、步驟、例外、待標準化問題與簽核。
- [ ] **T19-04 KPI**：定義、公式、單位、週期、來源、Baseline、Target、實績、偏差與資料品質。
- [ ] **T19-05 客戶導入／Onboarding**：貨主、倉別、階段、Checklist、預計上線日、阻塞、證據與放行結果。
- [ ] **T19-06 人員盤點**：人員、團隊、角色、技能、授權、可用狀態、訓練需求與資料有效期；禁止自動績效評分。
- [ ] **T19-07 跨模組查詢**：可由問題查看相關 KPI、SOP、Onboarding、容量與人員資料。
- [ ] **T19-08 管理 Dashboard**：六模組導覽、摘要、偏差、資料新鮮度、歷史與鑽取。

**驗收 Gate：** 六模組均能新增／查詢／修正／查看歷史與來源；同一資料不靠重複輸入維持；管理者可分辨「真實狀態、資料過期、尚待確認」。

---

## Phase 20｜執行與改善追蹤

**目標：** 將已確認的管理問題轉成可追蹤的改善事項，不讓 AI 直接指派或結案。

- [ ] **T20-01** 建立 `work_items` 與 `work_item_events`，包含來源、負責人、截止日期、狀態、改善結果與證據。
- [ ] **T20-02** 由管理者從營運問題、KPI 偏差、SOP 缺口或 Onboarding 阻塞手動建立任務。
- [ ] **T20-03** 建立接受、進行中、等待、完成、重新開啟與取消流程及 Audit。
- [ ] **T20-04** 建立可設定提醒；預設不對外發送，啟用前需確認對象、頻率與安靜時段。
- [ ] **T20-05** 改善完成時保存結果、前後差異、證據與關聯 KPI；不得只以一句「已完成」結案。
- [ ] **T20-06** 建立管理者 Review：逾期、阻塞、復發與無改善證據項目。
- [ ] **T20-07** 建立任務權限、通知退訂、敏感資料遮罩與完整稽核測試。

**驗收 Gate：** 每個任務有明確來源與人工確認紀錄；完成可查證、可重新開啟；AI 建議不會自行造成指派、通知或結案。

---

## 3. 目前建議 Sprint

1. T18-01～T18-03：先完成來源權威範圍、共用欄位與 Audit 契約。
2. T18-05～T18-06：完成總表匯入預覽、衝突處理與過渡策略。
3. T19-02＋T19-04：先做「營運問題＋KPI」，建立管理層最小閉環。
4. T19-01＋T19-05：加入倉庫容量與客戶導入。
5. T19-03＋T19-06：加入 SOP 與人員盤點。
6. T19-07～T19-08：完成跨模組關聯與管理 Dashboard。
7. Phase 19 驗收後，才啟動 Phase 20 執行與改善追蹤。

Sprint Demo 必須展示：同一營運問題可回查總表或事實來源、KPI 定義與實績可分離、資料過期或衝突會被標示、跨模組關聯不靠重複輸入，且尚未人工確認的項目不會自行變成任務。
...
- 《貨達營運總表》已先採本次管理對話的整理結果作為 V1 初始資料；仍待確認長期更新責任人、匯出格式與同步頻率。
- 六個模組的首批 Baseline、KPI 公式、狀態字典與管理責任人。
- Phase 20 的提醒對象、頻率、安靜時段與任務權限；SLA 處分與員工績效仍不在範圍內。
